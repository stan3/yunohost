...
def user_permission_list(auth, app=None, permission=None, username=None, group=None):
    ...
    for res in result:
        ...
        if 'URL' in res:
            ...
            for p in res['URL']:
                permissions[app_name][permission_name]['URL'].append(p)
...
def permission_add(operation_logger, auth, app, permission, url=None, default_allow=True, sync_perm=True):
    ...
    Keyword argument:
        app        -- an application OR sftp, xmpp (metronome), mail
        permission -- name of the permission ("main" by default)
        url        -- list of url to secify for the permission
...
def permission_update(operation_logger, auth, app, permission, add_url=None, remove_url=None, sync_perm=True):
    ...
    # Populate premission informations
    ...
    if add_url:
        url = url.union(add_url)
    if remove_url:
        url = url.difference(remove_url)
